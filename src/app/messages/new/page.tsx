...
export default async function NewMessagePage() {
  ...
  return (
    ...
    >
      <Link
        href="/messages"
        className="tap-target mb-4 inline-flex min-h-11 items-center gap-2 text-sm text-ink-muted hover:text-white"
      >
      ...
      {customers.length === 0 ? (
        <section className="rounded-panel border border-line bg-surface p-8 text-center">
          <UserRoundX className="mx-auto h-8 w-8 text-ink-faint" aria-hidden />
            ...
            {customers.map((customer, index) => (
              <li key={customer.id}>
                <form action={startConversation}>
                  ...
                  >
                    <span
                      className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-white/[0.06] text-sm font-bold text-ink-muted"
                      aria-hidden
                    >
                      {customer.initials}
                    </span>

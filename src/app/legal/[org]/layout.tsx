...
export default async function TenantLegalLayout({
  ...
  return (
    <main className="min-h-screen bg-canvas px-4 py-10 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl">
        <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold">
          <Zap className="h-5 w-5 fill-[#ffbf18] text-[#ffbf18]" aria-hidden /> VOLTEIRA
        </Link>
        <article className="mt-6 rounded-panel border border-line bg-surface p-6 sm:p-8">
          {children}
        </article>
        <nav className="mt-6 flex gap-4 text-sm text-ink-muted">

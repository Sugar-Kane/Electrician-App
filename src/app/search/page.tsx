...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ query?: string; scope?: string }> }) {
  ...
  const jobs = allJobs.filter((job) => !normalized || `${job.customer} ${job.contactName} ${job.id} ${job.address} ${job.city} ${job.workType}`.toLowerCase().includes(normalized));
  ...
  return (
    <FieldPageShell title={title} eyebrow="Find anything" description="Search by customer, job number, address, city or work type.">
      <section className="rounded-panel border border-line bg-surface p-4 sm:p-6">
        {/*
          Materials are no longer offered here. `job.materials` is empty for
          every real job — parts live in job_line_items now — so a promise to
          search them matched nothing and looked like the part was missing.
        */}
        <form action="/search" className="flex gap-2"><input type="hidden" name="scope" value={scope ?? ""} /><label className="flex min-h-13 flex-1 items-center gap-3 rounded-control border border-line bg-raised px-4"><Search className="h-5 w-5 text-ink-faint" aria-hidden /><span className="sr-only">Search</span><input name="query" defaultValue={query} autoFocus placeholder="Name, address, job #…" className="min-w-0 flex-1 bg-transparent text-base text-white outline-none placeholder:text-ink-faint" /></label><button className="tap-target grid h-13 w-13 place-items-center rounded-control bg-brand text-on-brand" aria-label="Run search"><Search className="h-5 w-5" aria-hidden /></button></form>

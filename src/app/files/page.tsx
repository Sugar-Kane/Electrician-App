...
function FolderBranch({ node, depth = 0 }: { node: DocumentFolderNode; depth?: number }) {
  ...
  const content = (
    <div className="flex min-h-12 items-center gap-3 rounded-control px-3 py-2 text-left hover:bg-white/[0.035]">
      ...
      <span className="min-w-0 flex-1">
        ...
        {node.type === "job" ? <span className="block text-[10px] text-ink-faint">Standard job documentation</span> : null}
      </span>
      {hasChildren ? <ChevronRight className="h-4 w-4 rotate-90 text-ink-faint" aria-hidden /> : null}
    </div>
  );
    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
        ...
        <div className="space-y-4">
          ...
          <section id="setup" className="rounded-panel border border-line bg-surface p-5 sm:p-6">
          ...
          <section className="rounded-panel border border-line bg-surface p-5 sm:p-6">
            <div className="flex items-center gap-3"><RefreshCw className="h-5 w-5 text-ink-muted" aria-hidden /><div><p className="text-xs text-ink-faint">Additional provider</p><h2 className="font-semibold">OneDrive-compatible structure</h2></div></div>
            <p className="mt-3 text-sm leading-6 text-ink-muted">The same folder and sync records support OneDrive. Google Drive is implemented first so users never maintain two competing folder trees.</p>
          </section>
        </div>
      </div>
      ...
      <section className="mt-4 grid gap-3 sm:grid-cols-3">
        {[
          ...
          <div key={title} className="rounded-panel border border-line bg-surface p-4">

...
export default async function BookingConfirmationPage({
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#06131d] px-4 py-10 text-white">
        <section className="w-full max-w-lg rounded-[28px] border border-white/10 bg-[#081925] p-6 text-center sm:p-8">
          ...
          <Link href={`/book/${encodeURIComponent(slug)}`} className="tap-target mt-6 inline-flex min-h-12 items-center justify-center rounded-2xl border border-white/10 px-5 text-sm font-semibold">
            Return to booking
          </Link>
        </section>
      </main>
  ...
  return (
    <main className="min-h-screen bg-[#06131d] px-3 py-5 text-white sm:px-5 sm:py-8">
      <div className="mx-auto max-w-2xl">
        ...
        <section className="mt-8 overflow-hidden rounded-[30px] border border-white/10 bg-[#081925]">
          <div className="border-b border-emerald-400/15 bg-emerald-400/[0.05] p-6 text-center sm:p-8">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-400">Your diagnostic visit is confirmed. A receipt has been sent by Stripe.</p>
          </div>

          <div className="space-y-3 p-4 sm:p-6">
            <div className="flex items-start gap-3 rounded-2xl border border-white/10 bg-[#0d202d] p-4">
              <CalendarCheck2 className="mt-0.5 h-5 w-5 shrink-0 text-[#ffc21c]" aria-hidden />
              <div>
                <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Arrival window</p>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-white/10 bg-[#0d202d] p-4">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Service location</p>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-white/10 bg-[#0d202d] p-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Diagnostic payment</p>
          ...
          <div className="border-t border-white/10 p-5 sm:p-6">
            ...
            {confirmation.business_phone ? (
              <a href={`tel:${confirmation.business_phone}`} className="tap-target mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl border border-white/10 text-sm font-semibold text-white">
                <Phone className="h-4 w-4 text-[#ffc21c]" aria-hidden /> Call {confirmation.business_phone}
              </a>
            ) : null}
          </div>
        </section>

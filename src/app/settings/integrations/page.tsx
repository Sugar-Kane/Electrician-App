...
export default function SupplierIntegrationsPage() {
  ...
  return (
    <FieldPageShell
          ...
          return (
            <section key={supplier.id} className="rounded-panel border border-line bg-surface p-5 sm:p-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {supplier.capabilities.map((capability) => <span key={capability} className="flex min-h-10 items-center gap-2 rounded-chip bg-white/[0.035] px-3 text-xs text-ink-muted"><Check className="h-3.5 w-3.5 shrink-0 text-emerald-400" aria-hidden />{capability}</span>)}
              </div>
              ...
              <p className="mt-4 flex items-start gap-2 text-xs leading-5 text-ink-muted"><TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-amber-300" aria-hidden />{supplier.caveat}</p>
      ...
      <section className="mt-4 rounded-panel border border-emerald-400/20 bg-emerald-400/[0.045] p-5 sm:p-6">

...
function SaveButton({ publishing }: { publishing: boolean }) {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="tap-target flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#ffc21c] px-5 text-sm font-bold text-[#071723] disabled:cursor-wait disabled:opacity-70"
    >
    ...
    <form action={formAction} className="space-y-4">
      ...
      <section className="rounded-3xl border border-white/10 bg-[#0b1b27] p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block sm:col-span-2">
            <span className="text-sm font-medium text-slate-200">Mailing address</span>
            <input name="mailingAddress" defaultValue={page.mailingAddress} className={inputClass} />
          </label>
        </div>

        {usingLoginEmail ? (
          <p className="mt-4 flex items-start gap-2 rounded-2xl border border-amber-300/25 bg-amber-300/[0.07] p-4 text-sm leading-6 text-amber-100">
      ...
      <section className="rounded-3xl border border-white/10 bg-[#0b1b27] p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-200">Message frequency</span>
        ...
        {page.slug ? (
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="tap-row flex min-h-12 items-center justify-between rounded-2xl border border-white/10 px-4 text-sm font-semibold"
            >
              <span className="flex items-center gap-2">
                <Globe className="h-4 w-4 text-[#ffc21c]" aria-hidden /> Preview Privacy Policy
              </span>
            ...
            >
              <span className="flex items-center gap-2">
                <Globe className="h-4 w-4 text-[#ffc21c]" aria-hidden /> Preview Text Message Terms
              </span>
              <ExternalLink className="h-4 w-4 text-slate-500" aria-hidden />
            </a>
          </div>
      ...
      {state.error ? (
        <p role="alert" className="rounded-2xl border border-red-400/20 bg-red-400/[0.07] p-4 text-sm text-red-200">
          {state.error}
        </p>
      ) : null}
      {state.saved ? (
        <p className="flex items-center gap-2 rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.07] p-4 text-sm text-emerald-100">
